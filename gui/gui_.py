...
from tkinter import Label, Entry, LabelFrame, Button, Tk, WORD, END, BOTH, Y, LEFT, RIGHT
...
class _Sounds:
    def __init__(self):
        ...
        self._login_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_login1.ogg'))
        self._pointing_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_scroll1.ogg'))
        self._select_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_popup_select1.ogg'))
        self._success_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_button_on1.ogg'))
        self._error_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_button_ok1.ogg'))
        self._logout_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_logout1.ogg'))
        self._accept_sound = self._mixer.Sound(os.path.join(self._sounds_folder, 'gui_popup_open1.ogg'))
...
class _Texts:
    def __init__(self, sub_frames: _SubFrames):
        ...
        self.text_log_main_frame_widget = ScrolledText(master=self.sub_frames.text_log_main_frame, wrap=WORD,
                                                       state='disabled')
...
class _Commands:
    def __init__(self, sounds: _Sounds, entries: _Entries, labels: _Labels, main_frames: _MainFrames, events_hash: dict[str, Event]):
    ...
    def set_new_local_directory(self):
        ...
        if not new_directory:
            ...
            self._labels.label_info_change_message('green',
                                                   f'Local path has been changed\n'
                                                   f'FROM {old_value}\n'
                                                   f'TO {new_directory}')
        else:
            self._sounds.error_sound()
            self._labels.label_info_change_message('red', f'Directory by path {new_directory} not found')
    ...
    def set_new_interval(self):
        try:
            ...
            self._sounds.accept_sound()
            self._labels.label_info_change_message('green',
                                                   f'Interval has been changed from {old_value} to {new_value}')
    ...
    def toggle_full_screen(self):
        if self._main_frames.main_frame.attributes('-fullscreen'):
            ...
            self._main_frames.main_frame.geometry(self._main_frames.vertical + 'x' + self._main_frames.horizontal)
            self._main_frames.main_frame.attributes('-fullscreen', True)
            self._main_frames.main_frame.resizable(False, False)


class _Buttons:
    def __init__(self, entries: _Entries, labels: _Labels, commands: _Commands, sounds: _Sounds, sub_frames: _SubFrames):
        self._entries = entries
        self._labels = labels
        ...
        self.menu_buttons = [self[attribute_name] for attribute_name in dir(self)[::-1] if attribute_name.endswith('menu')]
        self.all_controller_widgets = [self[attribute_name] for attribute_name in dir(self) if attribute_name.startswith('button')]
...
class Gui:
    def __init__(self, queue: Queue, events_hash: dict[str, Event]):
        ...
        self._commands = _Commands(self._sounds, self._entries, self._labels, self._main_frames, self._events_hash)
        self._buttons = _Buttons(self._entries, self._labels, self._commands, self._sounds, self._sub_frames)
    ...
    def _colorize_word_in_last_line(self, word, color):
        last_line_text = self._texts.text_log_main_frame_widget.get(END + "-2 lines linestart", END).strip()
