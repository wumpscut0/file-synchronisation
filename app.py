...
class Synchronizer:
    ...
    def _check_local_folder_change(self):
        if self._events_hash["local_folder_set_event"].is_set():
            ...
            create_folder_response_status_code = self.create_folder(
                local_folder_name
            ).status_code
...
def load_local_file(
    file_name: str, queue: Queue, synchronizer: Synchronizer, overwrite=False
):
    parser.read(CONFIG_PATH)
...
def synchronization(
    synchronizer: Synchronizer, queue: Queue, events_hash: dict[str, Event]
):
    ...
    if os.path.getsize(os.path.abspath(record_keeping_path)) > 0:
        ...
        for last_file_name, last_file_size in last_local_data.items():
            if last_file_name not in local_data and delete_remote_file(
                last_file_name, queue, synchronizer
            ):
                changes_have_been_made = True
                record_keeping.pop(last_file_name)
            elif (
                last_file_name in local_data
                and local_data[last_file_name] != last_file_size
                and load_local_file(last_file_name, queue, synchronizer, overwrite=True)
            ):
                changes_have_been_made = True
                record_keeping[last_file_name] = local_data[last_file_name]

        for local_file_name, local_file_size in local_data.items():
            if local_file_name not in last_local_data and load_local_file(
                local_file_name, queue, synchronizer
            ):
                changes_have_been_made = True
...
def get_meta_data_files_local_folder(
    queue: Queue, events_hash: dict[str, Event]
) -> dict[str, int]:
    parser.read(CONFIG_PATH)
    ...
    while not events_hash["exit_event"].is_set():
        ...
        if os.path.exists(local_path):
            ...
            log = (
                f'Local directory {"not exists" if local_path else "not set"}.'
    ...
    try:
        local_files_hash.pop(
            parser.get("app_config", "record_keeping_path").split("/")[-1]
        )
    except KeyError:
        pass
...
def sleep_by_interval(events_hash: dict[str, Event]):
    ...
    while (
        time() - start_wait < interval
        and not events_hash["interval_set_event"].is_set()
        and not events_hash["exit_event"].is_set()
    ):
        pass
